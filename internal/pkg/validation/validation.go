...
var rEmail = regexp.MustCompile("^[a-zA-Z0-9.!#$%&'*+/=?^_{|}~-]+@[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)*\\.[a-z]{2,6}$")
var rPassword = regexp.MustCompile("^[a-zA-Z0-9]{6,}$")
...
func Email(s *string, allowNil bool) bool {
	if allowNil {
		return s == nil || rEmail.MatchString(*s)
	}
	return s != nil && rEmail.MatchString(*s)
}
